...
export const startGameHandler = (id) => {
  ...
  let text;
  fetch(`https://bsa-patterns.herokuapp.com/game/texts/${id}`)
    .then((res) => res.json())
  ...
  reference = (e) => {
    if (e.key === text[counter]) {
      ...
      socket.emit("pressed_key", {
        percentage: ((counter + 1) * 100) / text.length,
        roomName,
      });
      counter++;
      textWrapper.childNodes[counter]?.classList.add("next");
      console.log(text.length - counter === 30);
      if (text.length - counter === 30) {
        socket.emit("close_to_finish", roomName);
      }
    }
